...
def lin_comb_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    colm = mat2coldict(M)
    return sum([v[x]*colm[x] for x in M.D[1]])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    eowm = mat2rowdict(M)
    return sum([v[x]*rowm[x] for x in M.D[0]])
...
def dot_product_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    rowm = mat2rowdict(M)
    return Vec(M.D[0],{x:v*rowm[x] for x in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    colm = mat2coldict(M)
    return Vec(M.D[1],{x:v*nm[x] for x in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    colm = mat2coldict(B)
    return coldict2mat({x:A*nm[x] for x in B.D[1]})
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    rowa = mat2rowdict(A)
    return rowdict2mat({x:rowa[x]*B for x in A.D[0]})
