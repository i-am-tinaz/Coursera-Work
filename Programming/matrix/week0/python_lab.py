...
first_five_pows_two = { 2**z for z in {0,1,2,3,4} }
...
first_of_tuples_list = [(i,j,k) for i in S for j in S for k in S if i+j+k==0][1]
...
odd_num_list_range = {2*x+1 for x in list(range(50))}
...
letter = ['A','B','C','D','E']
range_and_zip = list(zip(range(5),letter))
...
value_list_modified_1 = [x[k] for x in dlist if k in x] # <-- Use the same expression here
k = 'Frodo'
...
square_dict = {x:x*x for x in list(range(100))}
...
def nextInts(L): return [L[x]+1 for x in list(range(len(L))) ]



## 27: (Task 27) Procedure cubes
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def cubes(L): return [L[x]**3 for x in list(range(len(L))) ]
...
def list2dict(L, keylist): return {keylist[x]:L[x] for x in list(range(len(L)))}
